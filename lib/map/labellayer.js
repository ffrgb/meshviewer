  ...
  function (L, rbush, helper) {
    ...
    return L.GridLayer.extend({
      ...
      prepareLabels: function () {
        ...
        var labelsOffline = d.offline.map(prepareLabel(config.color.map.label.offline, 9, 5, false));
        var labelsNew = d.new.map(prepareLabel(config.color.map.label.new, 11, 8, true));
        var labelsLost = d.lost.map(prepareLabel(config.color.map.label.lost, 11, 8, true));
