define(['d3-selection', 'd3-force', 'd3-zoom', 'd3-drag', 'utils/math',  'utils/scales', 'forcegraph/draw'],
  function (d3Selection, d3Force, d3Zoom, d3Drag, math, scales, draw) {
    'use strict';
    ...
    return function (sidebar, router) {
      ...
      function onClick() {
        ...
        var n = force.find(e[0], e[1], config.constance.forcegraph.node.radiusSelect);
        ...
        var closedLink;
        var radius = config.constance.forcegraph.link.radiusSelect;
        intLinks
          .forEach(function (d) {
      ...
      forceLink = d3Force.forceLink()
        .distance(function (d) {
          if (d.o.vpn) {
            return config.constance.forcegraph.force.distance.vpn;
          }
          return config.constance.forcegraph.force.distance.other;
        })
        .strength(function (d) {
          if (d.o.vpn) {
            return config.constance.forcegraph.force.strength.vpn;
          }
          return Math.max(config.constance.forcegraph.force.strength.other, 1 / d.o.tq);
        });

      var zoom = d3Zoom.zoom()
        .scaleExtent([config.constance.forcegraph.zoomMin, config.constance.forcegraph.zoomMax])
        .on('zoom', function () {
      ...
      force = d3Force.forceSimulation()
        ...
        .force('x', d3Force.forceX().strength(config.constance.forcegraph.force.strength.center))
        .force('y', d3Force.forceY().strength(config.constance.forcegraph.force.strength.center))
      ...
      var drag = d3Drag.drag()
        .subject(function () {
          ...
          var n = force.find(e[0], e[1], config.constance.forcegraph.node.radiusDrag);
        ...
        .on('start', function () {
          if (!d3Selection.event.active) {
            force.alphaTarget(config.constance.forcegraph.force.alpha).restart();
          }
      ...
      self.resetView = function resetView() {
        ...
        transform.k = (config.constance.forcegraph.zoomMin + 1) / 2;
        moveTo(0, 0);
        redraw();
      };
      ...
      self.gotoNode = function gotoNode(d) {
        for (var i = 0; i < intNodes.length; i++) {
          ...
          transform.k = (config.constance.forcegraph.zoomMax + 1) / 2;
          moveTo(n.x, n.y);
          break;
        }
        redraw();
