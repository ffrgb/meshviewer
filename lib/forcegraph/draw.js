define(['helper'], function (helper) {
  ...
  function drawDetailNode(d) {
    if (transform.k > 1) {
      ...
      ctx.fillStyle = config.color.forcegraph.clients;
      ctx.fill();
      ctx.beginPath();
      ...
      ctx.fillStyle = config.color.forcegraph.label;
  ...
  function drawHighlightNode(d) {
    if (highlight && highlight.type === 'node' && d.o.node === highlight.o) {
      ctx.arc(d.x, d.y, config.constance.forcegraph.node.radiusHightlight, 0, 2 * Math.PI);
      ctx.fillStyle = config.color.forcegraph.highlight;
      ctx.fill();
      ctx.beginPath();
    }
  }
  ...
  function drawHighlightLink(d, to) {
    if (highlight && highlight.type === 'link' && d.o === highlight.o) {
      ...
      ctx.strokeStyle = config.color.forcegraph.highlight;
      ctx.lineWidth = config.constance.forcegraph.link.radiusHightlight;
      ctx.lineCap = 'round';
      ctx.stroke();
  ...
  self.drawNode = function drawNode(d) {
    ...
    ctx.arc(d.x, d.y, config.constance.forcegraph.node.radiusDraw, 0, 2 * Math.PI);
    ctx.fillStyle = config.color.forcegraph.node;
    ctx.fill();
