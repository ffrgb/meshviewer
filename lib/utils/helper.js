  ...
  positionClients: function positionClients(ctx, p, startAngle, node, startDistance) {
    ...
    for (var orbit = 0, i = 0; i < node.clients; orbit++) {
      ...
      for (var j = 0; j < Math.min(delta, n); i++, j++) {
        if (i >= (node.clients - node.clients_wifi5 - node.clients_other) && ctx.fillStyle !== 'rgba(147, 0, 220, 0.7)') {
          ctx.fill();
          ctx.beginPath();
          ctx.fillStyle = 'rgba(147, 0, 220, 0.7)';
        }
        if (i >= (node.clients - node.clients_other) && ctx.fillStyle !== 'rgba(220, 128, 0, 0.7)') {
          ctx.fill();
          ctx.beginPath();
