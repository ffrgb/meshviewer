  ...
  function (Chroma, V, Filter, helper) {
    ...
    return function (config, filterManager) {
      ...
      self.setData = function (data) {
        ...
        fillTable( _.t("stats.status"), statusTable, statusDict.sort(function (a, b) {
          return b[1] - a[1];
        }));
        fillTable( _.t("stats.firmware"), fwTable, fwDict.sort(function (a, b) {
          if (b[0] < a[0]) {
            return -1;
          }
          if (b[0] > a[0]) {
        ...
        fillTable(_.t("stats.hardware"), hwTable, hwDict.sort(function (a, b) {
          return b[1] - a[1];
        }));
        fillTable(_.t("stats.visible"), geoTable, geoDict.sort(function (a, b) {
          return b[1] - a[1];
        }));
        fillTable(_.t("stats.update"), autoTable, autoDict.sort(function (a, b) {
          return b[1] - a[1];
        }));
        fillTable(_.t("stats.site"), siteTable, siteDict.sort(function (a, b) {
          return b[1] - a[1];
        }));
      };
      ...
      self.render = function (el) {
        var h2;
        self.renderSingle(el, _.t("stats.status"), statusTable);
        self.renderSingle(el, _.t("stats.firmware"), fwTable);
        self.renderSingle(el, _.t("stats.hardware"), hwTable);
        self.renderSingle(el, _.t("stats.visible"), geoTable);
        self.renderSingle(el, _.t("stats.update"), autoTable);
        self.renderSingle(el, _.t("stats.site"), siteTable);
